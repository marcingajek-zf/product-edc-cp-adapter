...
public class BaseSqlDialectQueueStatements implements QueueStatements {
    ...
    public String getSaveMessageTemplate() {
        return format("INSERT INTO %s (%s, %s, %s, %s, %s) VALUES(?, ?, ?, ?%s, ?)",
            getQueueTable(), getCreatedAtColumn(), getIdColumn(), getChannelColumn(), getMessageColumn(), getInvokeAfterColumn(), getFormatJsonOperator());
    }
    ...
    public String getMessagesToSendTemplate() {
        return format("SELECT * FROM %s WHERE %s <= ? AND %s IS NULL LIMIT ?", getQueueTable(), getInvokeAfterColumn(), getLeaseIdColumn());
    };
    ...
    public String getUpdateTemplate() {
        return format("UPDATE %s SET %s=?, %s=?%s, %s=? WHERE %s=?",
            getQueueTable(), getChannelColumn(), getMessageColumn(), getFormatJsonOperator(), getInvokeAfterColumn(), getIdColumn());
    }
    ...
    public String getUpdateLeaseTemplate() {
        return format("UPDATE %s SET %s=? WHERE %s = ?;", getQueueTable(), getLeaseIdColumn(), getIdColumn());
    }
